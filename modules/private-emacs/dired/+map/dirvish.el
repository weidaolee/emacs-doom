...
(map!
 ...
 :n "yy" #'+dirvish/yank
 :n "p"  #'+dirvish/paste
 ;; ranger-style z* toggles. Doom binds single-key `z' to
 ;; `dirvish-history-jump' from inside (use-package! dirvish :config ...), i.e.
 ;; while dirvish loads, so it is always in place before this file runs from
 ;; `eval-after-load'. `z' has to be freed first or define-key refuses the
 ;; two-key sequence with "Key sequence z h starts with non-prefix key z"
 ;; (general's :prefix does not help -- it just concatenates the strings).
 ;; history-jump comes back on zj.
 :n "z"  nil
